...
def get_weather_forecast(citta):
    try:
        # Geocoding per ottenere lat/lon
...
def load_data():
    try:
        # Carichiamo il file con il nuovo nome richiesto
        ...
        st.error(f"Errore: Assicurati che il file 'Per viaggiare.csv' sia presente su GitHub. Errore: {e}")
        return None
...
if df_master is not None:
    ...
    # Input date invece del numero di giorni
    d_inizio = st.sidebar.date_input("Data Arrivo", date.today())
    d_fine = st.sidebar.date_input("Data Partenza", date.today().replace(day=date.today().day+7) if date.today().day < 20 else date.today())
    ...
    cane_on = st.sidebar.toggle("Cane presente?", value=True)
    ...
    def calcola_smart(row, giorni, pioggia, vento):
        # Filtro Hotel / Appartamento (Colonna specifica)
        tipo_casa = str(row.get('Hotel / Appartamento / Entrambi', 'Entrambi'))
        if alloggio == "Hotel" and tipo_casa == "Appartamento": return 0
        if alloggio == "Appartamento" and tipo_casa == "Hotel": return 0
        
        # Filtro Contesto Viaggio
        ...
        if "cane: cibo secco" in ogg: return f"{100 * giorni}g"
    ...
    st.title(f"🧳 Valigia per {citta}")
    st.subheader(f"Dal {d_inizio.strftime('%d/%m')} al {d_fine.strftime('%d/%m')} ({giorni} giorni)")

    # Sezione Raccomandazioni (Estrae righe con parole chiave)
    with st.expander("⚠️ Raccomandazioni importanti dal file"):
        recs = df_master[df_master['Oggetto'].str.contains('raccomandazioni|importante|attenzione|nota', case=False, na=False)]
        if not recs.empty:
            for r in recs['Oggetto']: st.write(f"• {r}")
        else:
            st.write("Nessuna raccomandazione specifica trovata.")
    ...
    for i, tab in enumerate(tabs):
        with tab:
            nome = nomi_p[i]
            if nome == "Comune":
                mask = (df_master['Proprietario'] == 'Comune') | (df_master['Proprietario'].str.contains('Cane', na=False))
            else:
