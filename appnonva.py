...
def salva_stato():
    stato_corrente = {
        ...
        "tipo_v": tipo_v,
        "alloggio": alloggio,
        "cane_presente": cane_presente,
        "override_cielo": override_cielo,
        "override_vento": override_vento,
    }
    st.session_state.saved_state = stato_corrente
    st.success("Valigia salvata ✅")
...
def reset_stato():
    ...
    st.success("Compilazione resettata 🔄")
...
st.sidebar.divider()
col_salva, col_reset = st.sidebar.columns(2)

with col_salva:
    st.button("💾 Salva", use_container_width=True, on_click=salva_stato)

with col_reset:
    st.button("🔄 Reset", use_container_width=True, on_click=reset_stato)

# Ripristino automatico stato salvato
if st.session_state.get("saved_state"):
    stato = st.session_state.saved_state
    citta = stato.get("citta", citta)
    d_inizio = stato.get("d_inizio", d_inizio)
    d_fine = stato.get("d_fine", d_fine)
    tipo_v = stato.get("tipo_v", tipo_v)
    alloggio = stato.get("alloggio", alloggio)
    cane_presente = stato.get("cane_presente", cane_presente)
    override_cielo = stato.get("override_cielo", override_cielo)
    override_vento = stato.get("override_vento", override_vento)
